...
/**
 * Refreshes the stalest followed series. Invoked by the worker's cron
 * trigger (see custom-worker.ts) with the CRON_SECRET header; it is also
 * reachable over HTTPS, hence the guard. Metadata ingestion stays the only
 * code path that talks to the provider (ADR-0004) — this just decides who
 * is due.
 */
